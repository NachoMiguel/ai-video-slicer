...
TESTING_MODE_CONFIG = {
    'enable_testing_mode': True,  # Set to True to enable testing mode
    'saved_script_path': 'SAVED_GENERATED_SCRIPT.md',  # Path to save/load scripts
    'skip_script_generation': False,  # Set to True to skip script generation and use saved script
    'auto_save_scripts': True,  # Automatically save generated scripts
}
...
# Script Management Functions for Testing Mode
def save_generated_script(script: str, video_title: str = "", transcript: str = "", metadata: dict = None):
    """Save generated script to file for testing mode reuse."""
    if not TESTING_MODE_CONFIG['enable_testing_mode'] or not TESTING_MODE_CONFIG['auto_save_scripts']:
        return
    
    script_path = TESTING_MODE_CONFIG['saved_script_path']
    timestamp = datetime.datetime.now().isoformat()
    
    content = f"""# Generated Script - {timestamp}
...
def load_saved_script():
    """Load saved script from file for testing mode."""
    script_path = TESTING_MODE_CONFIG['saved_script_path']
    ...
    try:
        ...
        script_start = content.find("## Generated Script\n\n")
        script_end = content.find("\n\n---\n\n## Original Transcript")
        
        if script_start == -1 or script_end == -1:
            print(f"[ERROR] Invalid saved script format in {script_path}")
            return None
            
        script = content[script_start + len("## Generated Script\n\n"):script_end].strip()
        print(f"[TESTING] Loaded saved script from {script_path} ({len(script)} characters)")
        return script
        
    except FileNotFoundError:
        print(f"[ERROR] Saved script not found at {script_path}")
        return None
    except Exception as e:
        print(f"[ERROR] Failed to load saved script: {e}")
        return None

def get_testing_mode_status():
    """Get current testing mode configuration status."""
    return {
        "testing_mode_enabled": TESTING_MODE_CONFIG['enable_testing_mode'],
        "skip_script_generation": TESTING_MODE_CONFIG['skip_script_generation'],
        "auto_save_scripts": TESTING_MODE_CONFIG['auto_save_scripts'],
        "saved_script_path": TESTING_MODE_CONFIG['saved_script_path'],
        "saved_script_exists": os.path.exists(TESTING_MODE_CONFIG['saved_script_path'])
    }
...
async def generate_script_from_youtube(
    ...
    skip_script_generation: bool = Form(False),
    save_script: bool = Form(True)
):
    print(f"[DEBUG] Starting generate_script_from_youtube")
    print(f"[DEBUG] youtube_url: {youtube_url}")
    print(f"[DEBUG] use_default_prompt: {use_default_prompt}")
    print(f"[DEBUG] custom_prompt: {custom_prompt}")
    
    try:
        # Using OpenAI account 1 directly (simplified approach)
        print("[DEBUG] Using OpenAI Account 1 for script generation")
        
        try:
            account_manager = ElevenLabsAccountManager()
            print("[DEBUG] ElevenLabs account manager initialized")
        except Exception as e:
            print(f"[ERROR] Failed to initialize ElevenLabs account manager: {e}")
            raise HTTPException(status_code=500, detail="TTS service initialization failed")
        
        print(f"[DEBUG] Creating temporary directory...")
        # Download YouTube video
        with tempfile.TemporaryDirectory() as temp_dir:
            print(f"[DEBUG] Temp dir created: {temp_dir}")
            
            print(f"[DEBUG] Setting up yt-dlp...")
            
            # Configure yt-dlp options - download raw audio without post-processing
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    try:
                        ...
                        print(f"[DEBUG] Video title: {video_title.encode('ascii', 'ignore').decode('ascii')}")
                    ...
                    print(f"[DEBUG] Audio downloaded to: {actual_audio_path}")
                    
            except Exception as yt_error:
                try:
                    print(f"[ERROR] Failed to download YouTube video: {yt_error}")
                except UnicodeEncodeError:
                    print("[ERROR] Failed to download YouTube video: [Unicode error in video title]")
                
                # Clean the error message for API response
                error_msg = str(yt_error)
                try:
                    error_msg.encode('ascii')
                except UnicodeEncodeError:
                    error_msg = "Failed to download video - video title contains unsupported characters"
                
                raise HTTPException(status_code=400, detail=f"Failed to access or download YouTube video: {error_msg}")
            ...
            try:
                ...
                try:
                    print(f"[DEBUG] First 200 chars: {transcript[:200]}...")
                except UnicodeEncodeError:
                    print("[DEBUG] First 200 chars: [Contains Unicode characters - preview skipped]")
            ...
            # Check if we should skip script generation and use saved script
            if skip_script_generation or TESTING_MODE_CONFIG['skip_script_generation']:
                print(f"[TESTING] Skipping script generation, loading saved script...")
                script = load_saved_script()
                if not script:
                    print(f"[ERROR] No saved script found, falling back to generation")
                    skip_script_generation = False
                else:
                    print(f"[TESTING] Using saved script ({len(script)} characters)")
            
            if not skip_script_generation and not TESTING_MODE_CONFIG['skip_script_generation']:
                print(f"[DEBUG] Preparing prompt...")
                prompt = DEFAULT_PROMPT if use_default_prompt else custom_prompt
                if not prompt:
                    print(f"[ERROR] No prompt provided")
                    raise HTTPException(status_code=400, detail="No prompt provided")
                
                print(f"[DEBUG] Using prompt: {prompt[:100]}...")
                print(f"[DEBUG] Transcript length: {len(transcript)} characters")
                print(f"[DEBUG] About to call OpenAI API using account 1...")
                
                try:
                    # Use account 1 directly instead of account switching
                    api_key_1 = os.getenv("OPENAI_API_KEY_1")
                    if not api_key_1:
                        raise Exception("OPENAI_API_KEY_1 not found in environment variables")
                    
                    # Create OpenAI client for account 1
                    from backend.openai_account_manager import create_openai_client
                    client_1 = create_openai_client(api_key_1)
                    ...
                    print(f"[DEBUG] OpenAI API call successful using account {used_account_id}")
                    print(f"[DEBUG] Token usage - Total: {response.usage.total_tokens}, Prompt: {response.usage.prompt_tokens}, Completion: {response.usage.completion_tokens}")
                    
                    # Save the generated script for future testing
                    if save_script and TESTING_MODE_CONFIG['enable_testing_mode']:
                        metadata = {
                        ...
                        save_generated_script(script, video_title, transcript, metadata)
            ...
            print(f"[DEBUG] Final script length: {len(script)} characters")
        ...
        print(f"[ERROR] Exception in generate_script_from_youtube: {e}")
...
async def get_testing_mode_status_endpoint():
    """Get current testing mode configuration and saved script status."""
    try:
        status = get_testing_mode_status()
        
        # Add additional information if saved script exists
        if status['saved_script_exists']:
            try:
                with open(status['saved_script_path'], 'r', encoding='utf-8') as f:
                    content = f.read()
                    
                # Extract metadata from saved script
                lines = content.split('\n')
                title_line = next((line for line in lines if line.startswith('**Title:**')), '')
                generated_line = next((line for line in lines if line.startswith('**Generated:**')), '')
                script_length_line = next((line for line in lines if line.startswith('**Script Length:**')), '')
                
                status['saved_script_info'] = {
                    'title': title_line.replace('**Title:**', '').strip(),
                    'generated': generated_line.replace('**Generated:**', '').strip(),
                    'length': script_length_line.replace('**Script Length:**', '').strip()
                }
            except Exception as e:
                status['saved_script_info'] = {'error': f'Could not read script info: {e}'}
        
        return status
    except Exception as e:
        return {"error": str(e)}

@app.post("/api/testing/configure")
async def configure_testing_mode(
    skip_script_generation: bool = Form(None),
    auto_save_scripts: bool = Form(None)
):
    """Configure testing mode settings."""
    try:
        if skip_script_generation is not None:
            TESTING_MODE_CONFIG['skip_script_generation'] = skip_script_generation
            print(f"[TESTING] Skip script generation set to: {skip_script_generation}")
        
        if auto_save_scripts is not None:
            TESTING_MODE_CONFIG['auto_save_scripts'] = auto_save_scripts
            print(f"[TESTING] Auto save scripts set to: {auto_save_scripts}")
        
        return {
            "status": "success",
            "message": "Testing mode configuration updated",
            "current_config": get_testing_mode_status()
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}

@app.get("/api/testing/script")
async def get_saved_script():
    """Get the content of the saved script for review."""
    try:
        script = load_saved_script()
        if script:
            return {
                ...
                "script": script,
                "length": len(script),
                "file_path": TESTING_MODE_CONFIG['saved_script_path']
            }
        else:
            return {
                "status": "error",
                "message": "No saved script found or script could not be loaded"
            }
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
from backend.script_analyzer import script_analyzer
...
async def set_entry_method(
    session_id: str = Form(...),
    entry_method: str = Form(...)  # 'youtube' or 'upload'
):
    """Set the entry method for a script session"""
    try:
        ...
        if entry_method not in ['youtube', 'upload']:
            raise HTTPException(status_code=400, detail="Invalid entry method. Must be 'youtube' or 'upload'")
        
        session.entry_method = EntryMethod(entry_method)
        session.current_phase = SessionPhase.EXTRACTION if entry_method == 'youtube' else SessionPhase.ANALYSIS
        
        session_manager.update_session(session)
        
        print(f"[DEBUG] Set entry method to {entry_method} for session {session_id}")
...
async def extract_youtube_transcript(
    ...
    try:
        ...
        session_manager.add_chat_message(
            session_id,
            "system",
            f"Successfully extracted transcript from YouTube video: {video_title}",
...
async def upload_script(
    ...
    """Upload and analyze an existing script"""
    try:
        session = session_manager.get_session(session_id)
        if not session:
            raise HTTPException(status_code=404, detail="Session not found")
        
        if session.entry_method != EntryMethod.UPLOAD:
            raise HTTPException(status_code=400, detail="Session not configured for upload entry method")
        
        # Get script content from form or file
        script_content = None
        filename = None
        ...
        if file and file.filename:
            ...
            print(f"[DEBUG] Uploaded script file: {filename}, length: {len(script_content)} characters")
        elif script:
            # Use script from form
            script_content = script
            filename = "pasted_script.txt"
            print(f"[DEBUG] Pasted script content, length: {len(script_content)} characters")
        else:
            raise HTTPException(status_code=400, detail="No script content provided")
        
        if len(script_content.strip()) < 100:
            raise HTTPException(status_code=400, detail="Script is too short (minimum 100 characters)")
        
        # Analyze the script
        print(f"[DEBUG] Analyzing uploaded script...")
        analysis = await script_analyzer.analyze_script(script_content, filename)
        
        # Update session with script data
        session.uploaded_script = script_content
        session.original_filename = filename
        session.script_analysis = analysis
        session.current_phase = SessionPhase.BUILDING
        ...
        session_manager.add_chat_message(
            session_id,
            "system",
            f"Successfully uploaded and analyzed script: {filename}",
            {
                "filename": filename,
...
async def generate_bullet_points(session_id: str = Form(...)):
    """Generate bullet points from transcript or uploaded script"""
    try:
        session = session_manager.get_session(session_id)
        if not session:
            raise HTTPException(status_code=404, detail="Session not found")
        ...
        # Prepare content based on entry method
        if session.entry_method == EntryMethod.YOUTUBE:
            ...
            prompt = load_prompt_from_file() if session.use_default_prompt else session.custom_prompt
            
        elif session.entry_method == EntryMethod.UPLOAD:
            if not session.uploaded_script:
                raise HTTPException(status_code=400, detail="No script uploaded")
            
            content = session.uploaded_script
            context = f"Uploaded Script:\n{content}"
            prompt = """
            ...
            raise HTTPException(status_code=400, detail="Invalid entry method")
...
async def handle_generate_section_command(session: ScriptSession, section_identifier: str):
    ...
    try:
        ...
        if session.entry_method == EntryMethod.YOUTUBE:
            ...
            context = f"Original Script: {session.uploaded_script[:1000]}..."
    ...
    try:
        ...
        return {
            ...
            "session": {
                ...
                "uploaded_script": session.uploaded_script,
                ...
                "completion_percentage": session.completion_percentage,
                "script_analysis": {
                    "structure_score": session.script_analysis.structure_score,
                    "readability_score": session.script_analysis.readability_score,
                    "suggestions": session.script_analysis.suggestions,
                    "key_points": session.script_analysis.key_points
                } if session.script_analysis else None,
